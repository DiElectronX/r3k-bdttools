...
if __name__ == "__main__":
    ...
    # read inputs
    filedir="/eos/cms/store/group/phys_bphys/DiElectronX/nzipper/Skims/Data_Skims"
    #filedir="/Users/noahzipper/Desktop/Research/Rk/Data_MC_Comparisons/RootFiles/CMG_Data_Ntuples"
    # files = sorted([(f, os.path.getsize(f)) for f in glob(filedir+'/**/*.root',recursive=True)], key=lambda l: l[1], reverse=True)
    files = glob(filedir+'/**/*.root',recursive=True)
    ...
    # which jobs to run
    jobFiles = np.array_split(np.array(files[:args.total]), args.nparts) if args.nparts>1 else files[:args.total]

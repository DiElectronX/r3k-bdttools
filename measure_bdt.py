...
def load_dir_args(args):
    with open(os.path.join(args.fromdir,'log.txt')) as f:
        for l in f:
            if 'Decay: ' in l: args.decay = l.split('Decay: ', 1)[1].strip()
            if 'Inputs: ' in l: args.features = ast.literal_eval(l.split('Inputs: ', 1)[1].strip())
            if 'Saving Model ' in l: args.model = l.split('Saving Model ', 1)[1].strip()
...
def evaluate_bdt(bdt, args, bdt_cols, out_cols, selection):
    ...
    with ur.open(args.measurefile) as datafile:
        data_dict = datafile['mytree'].arrays(bdt_cols, cut=selection if selection else None, library='np')
        out_branches = datafile['mytree'].arrays(out_cols, cut=selection if selection else None, library='np')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', dest='model', default='model.pkl', type=str, help='model name or file')
    parser.add_argument('--measurefile', dest='measurefile', default=None, type=str, help='file with events for measurement')
    parser.add_argument('--label', dest='label', default='', type=str, help='label for root file')
    parser.add_argument('--decay', dest='decay', default='kee', type=str, choices=['kmumu','kee'], help='decay type')
    parser.add_argument('--fromdir', dest='fromdir', default='', type=str, help='load params from designated model directory')
